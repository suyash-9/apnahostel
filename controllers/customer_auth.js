...
exports.login = async (req, res) => {
  try {
    ...
    if (!email || !password) {
      return res.status(400).render('customer/customerlogin', {
    ...
    db.query('SELECT * FROM customers WHERE email = ?', [email], async (error, results) => {
      ...
      if (results.length==0 || !(await bcrypt.compare(password, results[0].password))) {
        res.status(401).render('customer/customerlogin', {
...
exports.register = (req, res) => {
  ...
  db.query('SELECT email FROM customers WHERE email = ?', [email], async (error, results) => {
    ...
    if (results.length > 0) {
      return res.render('customer/customerregister', {
    ...
    db.query('INSERT INTO customers SET ?', { name: name, email: email, phone: phone, password: hashedPassword }, (error, results) => {
      if (error) {
        console.log(error);
      } else {
        console.log(results);
...
exports.isLoggedIn = async (req, res, next) => {
  ...
  if (req.cookies.jwt) {
    try {
      ...
      //2) Check if the customer still exists
