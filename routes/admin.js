...
router.post('/insert', admin_auth.isLoggedIn, function(req, res, next) { 
  ...
  if( req.admin ) {
    ...
    console.log(image)    
    db.query('INSERT INTO martyrs SET ?', { name:name, location:location, price:price, info:info, image:image }, (error, results) => {
      if (error) {
        console.log(error);
      } else {
        console.log(results);
  ...
  db.query(sql, function (err, data, fields) {
  if (err) throw err;
  res.render('admin/admindashboard', { title: 'Admin Dashboard',message: 'Listing Added', martyrData: data});
